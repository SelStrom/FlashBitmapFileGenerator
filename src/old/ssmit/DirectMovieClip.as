...
package old.ssmit
{
	...
	public class DirectMovieClip extends Sprite implements IAnimatable
	{
		...
		private var _lastLabelFrame		: int = -1;
		private var _firstLabelFrame	: int = 1;
		
		
		// Creates a new DirectMovieClip.
		public function DirectMovieClip()
		{
			frameRate = 60;
			_frameTime = 0;
			_globalFrame = -1;
		...
		// Cleans up everything excluding the textures.
		public override function dispose() : void
		{
			// ensure single dispose() call
			if ( _juggler == null ) return;
		...
		public function clone() : DirectMovieClip
		{
			var newMovieClip:DirectMovieClip = new DirectMovieClip();
...
//							targetFrame = _lastSceneFrame;
		...
		public function advanceTime( time:Number ) : void
		{
			if( _isPlaying )
			{
				...
				{
					++targetFrame;
					//trace("кадр изменился "+ targetFrame);					
					if (_lastLabelFrame != -1) {
						if( targetFrame > _lastLabelFrame )
						{
	//						if( loop )
								targetFrame = _firstLabelFrame;//анимация постоянно зациклена
	//						else
	//							targetFrame = _lastSceneFrame;
						}
					} else if (targetFrame > totalFrames) targetFrame = 1; 
					_frameTime -= frameDuration;
				}
			...
			// Advance the time of the children, even if this MovieClip is stopped. //строка нужна, чтобы проигрывать анимицию у потомков
			if(_juggler) _juggler.advanceTime( time ); //WARN скорее всего такой костыль будет вызывать утечку памяти, если  клип правильно не удалить из основного jugglera
		}
		...
		// Calls Sprite.flatten() on all children, excluding child DirectMovieClips, recursively.
		...
		public function flattenChildren() : void
		{
			for( var i:int=0; i<numChildren; ++i )
			{
				...
				if( child is DirectMovieClip )
					DirectMovieClip(child).flattenChildren();
		...
		private function getGlobalFrameFromLabel( labelName:String ) : int
		{
			...
			var size:int = _currentScene.labels.length;
			for (var i:int = 0; i < size ; i++) 
			{
				var label:FrameLabel = _currentScene.labels[i];
				
				if ( label.name == labelName )
				{
					if (i >= (size-1)) _lastLabelFrame = _lastSceneFrame;
					else _lastLabelFrame = _firstSceneFrame - 1 + _currentScene.labels[i+1] - 1;
		...
		public function gotoAndStop( frame:Object, scene:String = null ) : void
		{
			...
			//trace(this.name + " current frame is:" + this.currentFrame);
		}
...
// This class stores a single old.display object's properties, on a single frame.
