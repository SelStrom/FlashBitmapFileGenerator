...
package old.ssmit
{
	...
	{
		private static const TEXTURE_SIZE		: int   = 2048;
		private static const MAX_TEXELS			: int	= TEXTURE_SIZE * TEXTURE_SIZE;
		...
		internal static function pack( bitmapInfoList:Vector.<BitmapInfo>, sortBitmaps:Boolean=true ) : Vector.<BitmapData>
		{
			...
			{
				bitmapInfoList.sort(
					function ProcessSorting( info1:BitmapInfo, info2:BitmapInfo ) : Number
					{
			...
			for( var infoIndex:int=0; infoIndex<bitmapInfoList.length; ++infoIndex )
			{
				...
				if( info._bitmapData.width > TEXTURE_SIZE || info._bitmapData.height > TEXTURE_SIZE )//NOTE тут содержится проверка на размер текстуры
					throw new Error( "[Texture Packer] Bitmap is too large" ); 
				...
				for( var treeIndex:int=0; treeIndex<trees.length; ++treeIndex )
				{
					...
					{
						if( tree._width < TEXTURE_SIZE && tree._height < TEXTURE_SIZE )
						{
		...
		private static function calcTreeSize( bitmapInfoList:Vector.<BitmapInfo>, startIndex:int ) : int
		{
			...
			for( var i:int=startIndex; i<bitmapInfoList.length; ++i )
			{
				...
				// If the total texels has reached TEXTURE_SIZExTEXTURE_SIZE, then it's safe to say that should be the tree size.
